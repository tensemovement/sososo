...
## 8. 본문 가져와 재작성 + 대표 이미지 추출

각 항목에 대해 `WebFetch <url>` 로 본문을 가져온 뒤 **다시 쓴다.**

### 8-1. 재작성 규칙 (소소.소의 핵심)
본문 작성 규칙(톤·사실 보존·문단·`title`/`dek`/`body`/`tags` 필드별 제약 등)은 **단일 출처** 문서를 따른다.
다음 문서를 읽고 그대로 적용한다:

> **`.claude/shared/sososo-writing-rules.md`**

(이 규칙은 publish 스킬·refine 리뷰어와 공유한다. 규칙을 바꿀 때는 위 문서만 고친다.)
...
### 8-2. 대표 이미지 추출
우선순위 순으로 첫 매치를 `imageUrl` 로 채택:
1. 기사 HTML `<head>` 의 `og:image` → `og:image:url` → `twitter:image`
2. 위가 모두 없으면 **기사 본문(`<article>`/본문 영역)에서 가장 먼저 검출되는 `<img>` 의 `src`**
   - 로고·아이콘·1×1 트래킹 픽셀·광고 배너·이모지 등 비콘텐츠 이미지는 건너뛰고 다음 이미지를 본다.
- **반드시 `https://`**. `http://`·프로토콜 상대(`//...`)는 폐기.
- 경로만 있는 상대 URL 은 기사 origin 으로 절대화.
- 1·2 모두 실패·부재 시 `imageUrl` **필드 자체를 생략**(null·빈문자열 금지). UI 가 텍스트 카드로 렌더.
